...
完整逐秒数据边采集边追加到 `metrics_1s.csv`，不在内存中长期累积。运行时内存只保留最近 `runtime_memory_window_s`（默认 300 秒）；结束后流式读取 CSV 计算精确全程汇总，HTML/SVG 则每节点最多保留 `report_max_points_per_node`（默认 3600）个绘图点。
...
手动统计在浏览器中使用 HTML 内嵌的绘图抽样点即时计算，只改变当前页面显示，不修改服务器上的 CSV 或自动稳态结果。长时运行时手动区间是基于抽样点的可视化估算；`summary.csv` 和自动稳态统计仍使用完整逐秒 CSV。

手动时间曲线与“无稳态判断”保持相同口径：CPU利用率、功耗、内存利用率和整机功耗绘制单条实际曲线；CPU频率和温度绘制实际的平均值、最高值两条曲线；DCU指标按卡绘制，四卡节点显示四条实际曲线。图例中的平均值和最大值是手动区间统计文字，不会额外绘制成时间曲线。
...
| `runtime_memory_window_s` | 运行时仅在内存保留的近期样本时长，默认 300 秒 |
| `report_max_points_per_node` | HTML/SVG 每节点最大绘图点数，默认 3600 |
