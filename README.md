...
> 截图用于展示页面布局；当前版本可在全程、自动稳态和手动区间之间切换统计口径。
...
1. 可在“无稳态判断”“统一稳态区间”“每节点独立区间”和“手动截取区间”之间切换的平均值/最大值汇总表，默认使用全程数据
2. 跨节点关键指标对比
3. 可切换节点及四种统计口径的完整时间曲线

### 手动截取稳态区间

打开 `dashboard.html`，点击顶部的“手动截取区间”：

1. 选择“统一应用所有节点”，设置一次开始和结束时间，全部节点使用同一区间。
2. 或选择“每节点分别设置”，逐个选择节点并设置各自区间。
3. 控制区会直接显示DCU利用率波形：统一模式显示所有节点的四卡平均曲线；逐节点模式显示当前节点的四卡平均和单卡最大曲线。
4. 在同一条时间轨道上拖动开始、结束两个手柄，橙色区间会在波形图中实时同步；也可在组合输入框中填写相对脚本启动的秒数。
5. 点击“应用并查看”，节点汇总、跨节点比较和下方完整时间曲线会一起更新。

手动统计直接使用 HTML 中内嵌的原始采样数据，离线打开也能计算。它只改变当前页面显示，不修改原始 CSV、自动稳态结果或服务器上的任何文件。
...
- 统一口径：PD模式的 `auto` 会分别使用全部P节点、全部D节点生成候选区间，排除短于一个完整窗口的伪区间，再优先选择稳定带命中率更高、持续时间更长的候选；选中的同一区间用于所有节点。IFB模式使用全部IFB节点。
- 如果P、D都形成有效平台，程序会比较两个候选的稳定质量后自动择优；如果只有一组形成有效平台，就直接使用该组。因此P稳定、D间歇工作，或D稳定、P波动时，统一稳态都可以正常生成。
- `steady_state.json` 的 `group_candidates` 会保留P/D候选摘要，`auto_selected_group` 表示最终选中的组；HTML顶部、终端状态和结束日志也会显示AUTO选中了P还是D。
...
- HTML顶部提供“无稳态判断”“统一稳态区间”“每节点独立区间”“手动截取区间”四个按钮，默认选择“无稳态判断”。按钮会同时切换汇总表、跨节点比较和时间曲线，不需要修改JSONC或重新运行模型。
- 手动口径支持全部节点共用一个区间，也支持每个节点分别设置区间；平均值和最大值沿用自动汇总的同一计算口径，缺失值不按0计算。
...
| `steady_state.reference_group` | `auto` 在PD中自动比较P/D，也可填写 `P` 或 `D` 强制指定；IFB使用 `auto` |
