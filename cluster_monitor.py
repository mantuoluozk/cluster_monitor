...
def make_dashboard(path,model,node_infos,full_host,full_dcu,shared_host,shared_dcu,node_host,node_dcu,shared_steady,node_steady,started):
    ...
    for role in role_order:
        ...
        for item in role_items:
            ...
            scoped_images.append('''<div class="scope-content chart-scope manual-chart" data-scope="manual" data-node="%s"><span class="manual-chart-label">手动区间</span><div class="manual-dynamic-chart" role="img" aria-label="%s %s 手动区间动态监控曲线"></div></div>'''%
                                 (html.escape(item["node"],quote=True),html.escape(role),html.escape(item["node"])))
...
.manual-dynamic-chart{overflow:auto;border:1px solid #e3ebf1;background:#fff}.manual-dynamic-chart svg{display:block;width:100%;min-width:900px;height:auto}
