...
def _detect_steady_state_single(cfg,mode,active_groups,dcu_rows,total_elapsed):
    ...
    steady_values=[sample["util_pct"] for sample in series if start_elapsed<=sample["elapsed_s"]<=end_elapsed]
    in_band=[value for value in steady_values if abs(value-reference_mean)<=band]
    result["steady_sample_count"]=len(steady_values)
    result["in_band_sample_ratio"]=round(len(in_band)/len(steady_values),4) if steady_values else 0.0
    result["mean_absolute_deviation_pct"]=round(sum(abs(value-reference_mean) for value in steady_values)/len(steady_values),4) if steady_values else None
    return result


def _steady_candidate_summary(result):
    keys=("status","start_elapsed_s","end_elapsed_s","duration_s","start_confirmed","end_confirmed",
          "reference_group","reference_nodes","reference_mean_util_pct","stable_band_tolerance_pct",
          "steady_sample_count","in_band_sample_ratio","mean_absolute_deviation_pct","reason")
...
def detect_steady_state(cfg,mode,active_groups,dcu_rows,total_elapsed):
    ...
    if valid:
        # 平台内样本落在稳定带的比例优先，其次看持续时间和结束是否得到确认。
        selected_role,selected=max(valid.items(),key=lambda item:(
            float(item[1].get("in_band_sample_ratio") or 0),float(item[1].get("duration_s") or 0),
            1 if item[1].get("end_confirmed") else 0))
        result=dict(selected)
        result["auto_selection_reason"]="P/D分别判稳后选择稳定带命中率更高的有效区间；同等时优先持续时间更长者"
        ...
        result["reason"]="P/D均未形成至少%.1fs的有效稳定区间"%minimum_duration
        result["auto_selection_reason"]="P/D分别判稳，但目前没有可用于统一统计的候选区间"
        result["auto_selected_group"]=selected_role
    result["auto_candidate_min_duration_s"]=round(minimum_duration,3)
    result["group_candidates"]={role:_steady_candidate_summary(candidate) for role,candidate in candidates.items()}
    return result
...
def _dashboard_manual_series(host_rows,dcu_rows):
    ...
    host_defs={
        "cpu_util":("cpu_util_pct","cpu_util_pct"),"cpu_temp":("cpu_temp_avg_c","cpu_temp_max_c"),
        "cpu_power":("cpu_power_w","cpu_power_w"),"memory_used":("host_mem_used_gib","host_mem_used_gib"),
        "memory_util":("host_mem_util_pct","host_mem_util_pct"),"node_power":("node_power_w","node_power_w"),
    }
    for row in host_rows:
        elapsed=row.get("elapsed_s")
        if elapsed in (None,""):continue
        for key,(avg_key,max_key) in host_defs.items():
            avg_value=row.get(avg_key); max_value=row.get(max_key)
            if avg_value not in (None,"") or max_value not in (None,""):
                series[key].append([float(elapsed),None if avg_value in (None,"") else float(avg_value),
                                    None if max_value in (None,"") else float(max_value)])
    grouped=defaultdict(list)
    for row in dcu_rows:
        elapsed=row.get("elapsed_s")
        if elapsed not in (None,""):grouped[float(elapsed)].append(row)
    dcu_defs={"dcu_util":"dcu_util_pct","vram_util":"dcu_mem_util_pct","dcu_temp":"dcu_temp_c"}
    for elapsed,rows in sorted(grouped.items()):
        for key,column in dcu_defs.items():
            values=[float(row[column]) for row in rows if row.get(column) not in (None,"")]
            if values:series[key].append([elapsed,sum(values)/len(values),max(values)])
        powers=[float(row["dcu_power_w"]) for row in rows if row.get("dcu_power_w") not in (None,"")]
        if powers:
            total=sum(powers); series["dcu_power_total"].append([elapsed,total,total])
...
def make_dashboard(path,model,node_infos,full_host,full_dcu,shared_host,shared_dcu,node_host,node_dcu,shared_steady,node_steady,started):
    ...
    for item in overview:
        ...
        for key,_,_ in metric_defs:
            ...
            scope_values.append('<span class="scope-content manual-value" data-scope="manual" data-node="%s" data-metric="%s"><span class="avg">--</span><span class="max">--</span></span>'%
                                (html.escape(item["node"],quote=True),html.escape(key,quote=True)))
    ...
    for key,label,unit in comparison_defs:
        ...
        scope_blocks.append('<div class="scope-content manual-compare" data-scope="manual" data-manual-compare="%s"></div>'%html.escape(key,quote=True))
    ...
    for role in role_order:
        ...
        for item in role_items:
            ...
            scoped_images.append('''<div class="scope-content chart-scope manual-chart" data-scope="manual" data-node="%s"><span class="manual-chart-label">手动区间</span><div class="manual-chart-wrap"><img src="%s" loading="lazy" alt="%s %s manual monitoring chart"><i class="manual-band"></i></div></div>'''%
                                 (html.escape(item["node"],quote=True),html.escape(item["full_svg"]),html.escape(role),html.escape(item["node"])))
            detail_panels.append('''<article id="%s" class="detail-panel" data-role="%s" data-node="%s"><div class="detail-title"><div><span class="role role-%s">%s</span><h3>%s 完整时间曲线</h3></div></div>%s</article>'''%
                                 (detail_id,html.escape(role),html.escape(item["node"],quote=True),html.escape(role.lower()),html.escape(role),html.escape(item["node"]),"".join(scoped_images)))
    ...
    def describe_steady(steady,prefix):
        if steady and steady.get("start_elapsed_s") is not None:
            ...
            auto_note=(" · AUTO比较P/D后选中%s"%steady.get("auto_selected_group") if steady.get("auto_selected_group") else "")
            return "%s %.1fs–%.1fs · 持续 %.1fs · 结束%s%s"%(prefix,steady["start_elapsed_s"],steady["end_elapsed_s"],steady.get("duration_s",0),end_note,auto_note)
    ...
    manual_nodes={item["node"]:{"role":item["role"],**_dashboard_manual_series(full_host.get(item["node"],[]),full_dcu.get(item["node"],[]))} for item in overview}
    manual_payload=json.dumps({"started_epoch":started,"nodes":manual_nodes},ensure_ascii=False,separators=(",",":" )).replace("</","<\\/")
    node_options="".join('<option value="%s">%s / %s</option>'%(html.escape(item["node"],quote=True),html.escape(item["role"]),html.escape(item["node"])) for item in overview)
    replacements={
        ...
        "__MANUAL_DATA__":manual_payload,"__NODE_OPTIONS__":node_options,
    }
...
<style>
:root{--ink:#0b1f33;--muted:#5b7083;--line:#cbd8e4;--paper:#f4f7fa;--panel:#fff;--blue:#1769aa;--cyan:#008f95;--orange:#e05a24;--green:#16845b;--purple:#7857a8}*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--paper);color:var(--ink);font-family:"Microsoft YaHei UI","Microsoft YaHei",Arial,sans-serif}[hidden]{display:none!important}.page{width:min(1680px,100%);margin:auto;padding:20px 24px 44px}.masthead{display:flex;justify-content:space-between;gap:24px;align-items:flex-end;padding:22px 0 18px;border-bottom:3px solid var(--ink)}.eyebrow{font:700 12px Consolas,monospace;letter-spacing:.16em;color:var(--cyan)}h1{font-size:30px;margin:6px 0 0}.masthead p{margin:0;color:var(--muted)}.scope-switch{display:flex;gap:8px;align-items:center;padding:12px 0 4px;flex-wrap:wrap}.scope-switch strong{margin-right:4px}.scope-button{appearance:none;border:1px solid #8ca4b7;background:#fff;color:var(--ink);padding:9px 15px;cursor:pointer;font-weight:700}.scope-button.active{background:var(--ink);border-color:var(--ink);color:#fff}.anchor-nav{position:sticky;top:0;z-index:8;display:flex;gap:8px;padding:10px 0;background:rgba(244,247,250,.96);border-bottom:1px solid var(--line)}.anchor-nav a{padding:7px 11px;color:var(--blue);text-decoration:none;font-weight:700}.manual-panel{margin:14px 0 4px;border:1px solid #9fb5c7;border-left:5px solid var(--orange);background:#fff;padding:16px}.manual-panel h2{margin:0 0 5px;font-size:18px}.manual-help{margin:0 0 13px;color:var(--muted);font-size:13px}.manual-grid{display:grid;grid-template-columns:minmax(240px,.8fr) minmax(380px,1.5fr) auto;gap:18px;align-items:end}.manual-field label,.manual-mode-label{display:block;margin-bottom:6px;font-weight:700;font-size:13px}.manual-mode{display:flex;gap:7px;flex-wrap:wrap}.manual-mode button,.manual-apply,.manual-reset{border:1px solid #91a8ba;background:#fff;padding:8px 11px;cursor:pointer;font-weight:700}.manual-mode button.active{background:var(--blue);border-color:var(--blue);color:#fff}.manual-field select,.manual-field input[type=number]{width:100%;height:36px;border:1px solid #91a8ba;padding:5px 8px;background:#fff}.manual-node-field{margin-top:10px}.manual-range{display:grid;grid-template-columns:1fr 1fr;gap:10px}.range-inputs{grid-column:1/-1;display:grid;gap:2px;padding-top:5px}.range-inputs input{width:100%;accent-color:var(--orange)}.manual-clock{grid-column:1/-1;color:var(--muted);font:12px Consolas,monospace}.manual-actions{display:flex;gap:8px}.manual-apply{background:var(--orange);border-color:var(--orange);color:#fff}.manual-status{margin:12px 0 0;padding-top:10px;border-top:1px solid #e2eaf0;color:#314c61;font-size:13px}.section{margin-top:28px}.section-head{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:12px}.section h2{font-size:21px;margin:0}.section-head p{margin:0;color:var(--muted);font-size:13px}.table-wrap{overflow:auto;background:var(--panel);border:1px solid var(--line)}table{width:100%;min-width:1450px;border-collapse:collapse;font:13px Consolas,"Microsoft YaHei UI",monospace}th,td{padding:10px 9px;border-bottom:1px solid #e5edf3;text-align:right;white-space:nowrap}thead th{position:sticky;top:0;background:#eaf1f6;color:#243c50}thead th:first-child,tbody th{text-align:left;position:sticky;left:0;background:#fff;z-index:2}thead th:first-child{z-index:3;background:#eaf1f6}th small{display:block;color:var(--muted);font-weight:400}.avg{color:var(--ink)}.max{color:var(--orange);margin-left:7px}.max:before{content:"/ ";color:#9aa9b5}.role{display:inline-grid;place-items:center;min-width:34px;padding:3px 7px;margin-right:6px;border-radius:3px;color:#fff;font:700 12px Consolas,monospace}.role-p,.role-bg-p{background:var(--blue)}.role-d,.role-bg-d{background:var(--cyan)}.role-ifb,.role-bg-ifb{background:var(--purple)}.compare-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.compare-card{background:#fff;border:1px solid var(--line);padding:15px}.compare-card h3{margin:0;font-size:16px}.compare-card h3 small{color:var(--muted)}.compare-legend{margin:4px 0 12px;color:var(--muted);font-size:11px}.bar-row{display:grid;grid-template-columns:92px minmax(120px,1fr) 112px;gap:9px;align-items:center;margin:8px 0;font:12px Consolas,monospace}.bar-node{overflow:hidden;text-overflow:ellipsis}.role-dot{display:inline-block;width:7px;height:7px;margin-right:6px;border-radius:50%}.role-dot.role-p{background:var(--blue)}.role-dot.role-d{background:var(--cyan)}.role-dot.role-ifb{background:var(--purple)}.bar-track{height:13px;background:#e7eef3;position:relative}.bar-fill{display:block;height:100%;min-width:1px}.maximum-marker{position:absolute;top:-3px;transform:translateX(-50%);color:var(--orange)}.maximum-marker:after{content:"◆";font-style:normal;font-size:13px}.bar-value{text-align:right;color:#334e62}.detail-controls{background:#eaf1f6;border:1px solid var(--line);padding:12px}.role-tabs,.node-tabs{display:flex;flex-wrap:wrap;gap:8px}.node-tabs{margin-top:9px}.role-tab,.node-tab{appearance:none;border:1px solid #9fb2c1;background:#fff;color:var(--ink);padding:7px 12px;cursor:pointer;font:700 13px "Microsoft YaHei UI",sans-serif}.role-tab.active,.node-tab.active,.scope-button:focus-visible,.role-tab:focus-visible,.node-tab:focus-visible,a:focus-visible,button:focus-visible,input:focus-visible,select:focus-visible{outline:3px solid #f3a45f;outline-offset:2px}.role-tab.active,.node-tab.active{background:var(--ink);border-color:var(--ink);color:#fff}.detail-panel{display:none;margin-top:12px;background:#fff;border:1px solid var(--line);padding:14px}.detail-panel.active{display:block}.detail-title{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.detail-title>div{display:flex;align-items:center}.detail-title h3{display:inline;margin:0;font-size:18px}.svg-link{display:block;margin:-40px 0 14px auto;width:max-content;color:var(--blue);font-weight:700;text-decoration:none}.detail-panel img{display:block;width:100%;height:auto;border:1px solid #e3ebf1}.manual-chart-label{display:block;margin:0 0 8px;color:var(--purple);font-weight:700}.manual-chart-wrap{position:relative;overflow:hidden}.manual-band{position:absolute;top:0;bottom:0;background:rgba(224,90,36,.13);border-left:2px solid var(--orange);border-right:2px solid var(--orange);pointer-events:none}footer{margin-top:24px;color:var(--muted);font-size:12px}@media(max-width:900px){.page{padding:12px}.masthead{display:block}.masthead p{margin-top:9px}.scope-switch{align-items:stretch;flex-direction:column}.manual-grid{grid-template-columns:1fr}.compare-grid{grid-template-columns:1fr}.section-head{display:block}.section-head p{margin-top:5px}.bar-row{grid-template-columns:76px 1fr 92px}.detail-title{align-items:flex-start;gap:10px}.svg-link{margin:0 0 10px}}
.role-tab.active:not(:focus-visible),.node-tab.active:not(:focus-visible){outline:none}
.manual-preview{margin:14px 0;border:1px solid #c8d5df;background:#f8fafc}.manual-preview-head{display:flex;justify-content:space-between;gap:15px;align-items:center;padding:10px 13px;border-bottom:1px solid #dce5ec}.manual-preview-head h3{margin:0;font-size:14px}.manual-preview-head span{color:var(--muted);font:12px Consolas,monospace}.manual-preview svg{display:block;width:100%;height:250px}.manual-preview-legend{display:flex;flex-wrap:wrap;gap:8px 18px;padding:8px 13px;border-top:1px solid #dce5ec;color:#425d72;font:12px Consolas,monospace}.preview-key:before{content:"";display:inline-block;width:18px;height:3px;margin:0 6px 3px 0;background:var(--key-color)}.manual-grid{grid-template-columns:minmax(240px,.8fr) minmax(440px,1.5fr) auto}.manual-field select{width:100%;height:36px;border:1px solid #91a8ba;padding:5px 8px;background:#fff}.manual-range{display:grid;grid-template-columns:1fr;gap:9px}.interval-entry{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;border:1px solid #91a8ba;background:#fff}.interval-entry label{display:grid;grid-template-columns:auto 1fr;align-items:center;gap:7px;padding:5px 10px;color:var(--muted);font-size:12px}.interval-entry label+span{color:#8da0af}.interval-entry input{min-width:0;width:100%;height:26px;border:0!important;outline:0;background:transparent;color:var(--ink);text-align:right;font:700 14px Consolas,monospace}.dual-range{--range-start:0%;--range-end:100%;position:relative;height:30px;grid-column:1/-1}.dual-range-track{position:absolute;left:8px;right:8px;top:13px;height:6px;background:linear-gradient(to right,#d7e0e7 0 var(--range-start),var(--orange) var(--range-start) var(--range-end),#d7e0e7 var(--range-end) 100%)}.dual-range input{position:absolute;inset:0;width:100%;height:30px;margin:0;background:transparent;pointer-events:none;appearance:none;-webkit-appearance:none}.dual-range input::-webkit-slider-runnable-track{height:6px;background:transparent}.dual-range input::-moz-range-track{height:6px;background:transparent}.dual-range input::-webkit-slider-thumb{width:18px;height:22px;margin-top:-8px;border:3px solid #fff;border-radius:3px;background:var(--orange);box-shadow:0 0 0 1px #b94a1b,0 2px 5px rgba(11,31,51,.25);pointer-events:auto;cursor:ew-resize;appearance:none;-webkit-appearance:none}.dual-range input::-moz-range-thumb{width:14px;height:18px;border:3px solid #fff;border-radius:3px;background:var(--orange);box-shadow:0 0 0 1px #b94a1b,0 2px 5px rgba(11,31,51,.25);pointer-events:auto;cursor:ew-resize}.manual-clock{grid-column:1/-1}@media(max-width:900px){.manual-preview svg{height:210px}.manual-grid{grid-template-columns:1fr}}
</style></head><body><main class="page"><header class="masthead"><div><span class="eyebrow">SUPERNODE RESOURCE REPORT</span><h1>__TITLE__ 监控总览</h1></div><p>__NODE_COUNT__ 个计算节点<br><span class="scope-content" data-scope="full">__FULL_SUMMARY__</span><span class="scope-content" data-scope="shared">__SHARED_SUMMARY__</span><span class="scope-content" data-scope="per-node">__NODE_SUMMARY__</span><span class="scope-content" data-scope="manual" id="manual-summary">手动区间 · 等待设置</span></p></header><div class="scope-switch" role="group" aria-label="统计口径"><strong>统计口径</strong><button type="button" class="scope-button" data-select-scope="full">无稳态判断</button><button type="button" class="scope-button" data-select-scope="shared">统一稳态区间</button><button type="button" class="scope-button" data-select-scope="per-node">每节点独立区间</button><button type="button" class="scope-button" data-select-scope="manual">手动截取区间</button></div>
<section class="manual-panel" id="manual-panel" hidden><h2>手动截取稳态区间</h2><p class="manual-help">先观察DCU利用率波形，再拖动同一条轨道上的两个端点。橙色区域就是即将参与平均值和最大值计算的数据。</p><div class="manual-preview"><div class="manual-preview-head"><h3>DCU利用率参考曲线（%）</h3><span id="manual-preview-mode">全部节点四卡平均</span></div><svg id="manual-preview-svg" viewBox="0 0 1200 250" role="img" aria-label="手动区间DCU利用率预览"></svg><div class="manual-preview-legend" id="manual-preview-legend"></div></div><div class="manual-grid"><div><span class="manual-mode-label">区间应用方式</span><div class="manual-mode" role="group" aria-label="手动区间应用方式"><button type="button" data-manual-mode="shared">统一应用所有节点</button><button type="button" data-manual-mode="per-node">每节点分别设置</button></div><div class="manual-field manual-node-field" id="manual-node-field" hidden><label for="manual-node">当前节点</label><select id="manual-node">__NODE_OPTIONS__</select></div></div><div class="manual-range"><div class="interval-entry" aria-label="手动区间秒数"><label for="manual-start">开始<input id="manual-start" type="number" min="0" step="0.1"></label><span>→</span><label for="manual-end">结束<input id="manual-end" type="number" min="0" step="0.1"></label></div><div class="dual-range" id="manual-range-shell"><span class="dual-range-track"></span><input id="manual-start-range" type="range" min="0" step="0.1" aria-label="手动区间开始"><input id="manual-end-range" type="range" min="0" step="0.1" aria-label="手动区间结束"></div><div class="manual-clock"><span id="manual-start-clock">--</span> → <span id="manual-end-clock">--</span></div></div><div class="manual-actions"><button type="button" class="manual-apply" id="manual-apply">应用并查看</button><button type="button" class="manual-reset" id="manual-reset">重置为全程</button></div></div><p class="manual-status" id="manual-status" role="status">尚未应用手动区间。</p></section>
<nav class="anchor-nav"><a href="#overview">节点汇总</a><a href="#compare">跨节点比较</a><a href="#details">时间曲线</a></nav>
...
function loadControls(){const max=limit(),i=normalize(currentInterval(),max);[startRange,endRange].forEach(el=>el.max=max);startInput.max=endInput.max=max;startInput.value=i.start.toFixed(1);endInput.value=i.end.toFixed(1);startRange.value=i.start;endRange.value=i.end;rangeShell.style.setProperty('--range-start',(i.start*100/Math.max(max,.001)).toFixed(3)+'%');rangeShell.style.setProperty('--range-end',(i.end*100/Math.max(max,.001)).toFixed(3)+'%');startRange.style.zIndex=i.start>max*.85?'5':'3';endRange.style.zIndex='4';document.getElementById('manual-start-clock').textContent=clock(i.start);document.getElementById('manual-end-clock').textContent=clock(i.end);renderPreview()}
...
function recalc(){const results={};nodes.forEach(node=>{results[node]={};const interval=intervalFor(node);Object.entries(data.nodes[node].metrics).forEach(([key,points])=>results[node][key]=stats(points,interval))});document.querySelectorAll('.manual-value').forEach(el=>{const value=results[el.dataset.node][el.dataset.metric];el.querySelector('.avg').textContent=number(value.avg);el.querySelector('.max').textContent=number(value.max)});document.querySelectorAll('[data-manual-compare]').forEach(container=>{const key=container.dataset.manualCompare,scale=Math.max(1,...nodes.map(n=>results[n][key].max||0));container.innerHTML=nodes.map(node=>{const value=results[node][key],role=data.nodes[node].role.toLowerCase(),aw=value.avg==null?0:Math.min(100,value.avg*100/scale),ml=value.max==null?0:Math.min(100,value.max*100/scale);return '<div class="bar-row"><span class="bar-node"><i class="role-dot role-'+esc(role)+'"></i>'+esc(node)+'</span><div class="bar-track"><span class="bar-fill role-bg-'+esc(role)+'" style="width:'+aw.toFixed(3)+'%"></span>'+(value.max==null?'':'<i class="maximum-marker" style="left:'+ml.toFixed(3)+'%"></i>')+'</div><span class="bar-value">'+number(value.avg)+' / '+number(value.max)+'</span></div>'}).join('')});document.querySelectorAll('.manual-chart').forEach(chart=>{const node=chart.dataset.node,max=Math.max(0.001,data.nodes[node].max_elapsed_s),i=normalize(intervalFor(node),max),band=chart.querySelector('.manual-band');band.style.left=(6+92*i.start/max)+'%';band.style.width=(92*(i.end-i.start)/max)+'%';chart.querySelector('.manual-chart-label').textContent='手动区间 '+i.start.toFixed(1)+'s–'+i.end.toFixed(1)+'s（'+clock(i.start)+' → '+clock(i.end)+'）'});if(manualMode==='shared'){summary.textContent='手动统一区间 · '+sharedInterval.start.toFixed(1)+'s–'+sharedInterval.end.toFixed(1)+'s';status.textContent='已对全部 '+nodes.length+' 个节点应用统一区间：'+clock(sharedInterval.start)+' → '+clock(sharedInterval.end)}else{summary.textContent='手动节点独立区间 · '+nodes.length+' 个节点分别设置';const n=currentNode(),i=nodeIntervals[n];status.textContent='已按节点分别统计；当前 '+n+'：'+i.start.toFixed(1)+'s–'+i.end.toFixed(1)+'s'}showScope('manual')}
function showScope(scope){scopeButtons.forEach(b=>{const active=b.dataset.selectScope===scope;b.classList.toggle('active',active);b.setAttribute('aria-pressed',active)});scopeContents.forEach(el=>el.hidden=el.dataset.scope!==scope);manualPanel.hidden=scope!=='manual'}function showNode(id){nodeButtons.forEach(b=>b.classList.toggle('active',b.dataset.detail===id));panels.forEach(p=>p.classList.toggle('active',p.id===id))}function showRole(role){roleButtons.forEach(b=>b.classList.toggle('active',b.dataset.role===role));nodeGroups.forEach(g=>g.style.display=g.dataset.role===role?'flex':'none');const first=nodeButtons.find(b=>b.closest('.node-tabs').dataset.role===role);if(first)showNode(first.dataset.detail)}
scopeButtons.forEach(b=>b.addEventListener('click',()=>{showScope(b.dataset.selectScope);if(b.dataset.selectScope==='manual')loadControls()}));roleButtons.forEach(b=>b.addEventListener('click',()=>showRole(b.dataset.role)));nodeButtons.forEach(b=>b.addEventListener('click',()=>showNode(b.dataset.detail)));document.querySelectorAll('[data-manual-mode]').forEach(b=>b.addEventListener('click',()=>{manualMode=b.dataset.manualMode;document.querySelectorAll('[data-manual-mode]').forEach(x=>x.classList.toggle('active',x===b));nodeField.hidden=manualMode!=='per-node';loadControls()}));nodeSelect.addEventListener('change',loadControls);startInput.addEventListener('change',()=>saveControls('start-number'));endInput.addEventListener('change',()=>saveControls('end-number'));startRange.addEventListener('input',()=>saveControls('start-range'));endRange.addEventListener('input',()=>saveControls('end-range'));document.getElementById('manual-apply').addEventListener('click',()=>{commitControls();recalc()});document.getElementById('manual-reset').addEventListener('click',()=>{if(manualMode==='shared')sharedInterval={start:0,end:limit()};else nodeIntervals[currentNode()]={start:0,end:limit()};loadControls();recalc()});document.querySelector('[data-manual-mode="shared"]').classList.add('active');loadControls();recalc();showScope('full');if(roleButtons[0])showRole(roleButtons[0].dataset.role)})();</script></body></html>'''
...
def runtime_steady_status(cfg,mode,active_groups,roles,dcu_rows,total_elapsed):
    ...
    if all_nodes_idle:
        ...
        text="%s%s；全部节点DCU利用率≤%.1f%%，连续%d/%d个样本，可按 Ctrl+C"%(
            note,auto_note,idle,idle_needed,idle_needed)
    ...
    if steady.get("start_confirmed"):
        if idle_progress:
            text="已进入统一稳态（起点 %.1fs%s）；负载下降待确认 %d/%d"%(
                steady["start_elapsed_s"],auto_note,min(idle_progress,idle_needed),idle_needed)
        else:text="已进入统一稳态（起点 %.1fs%s）"%(steady["start_elapsed_s"],auto_note)
